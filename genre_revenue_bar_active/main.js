...
function setupCanvas(barChartData, moviesClean){
    ...
    function update(data){
        ...
        yScale = d3.scaleBand()
                   .domain(data.map(d=>cutText(d.title))) //對到每一筆資料的title
                   .rangeRound([0, chart_height])
                   .paddingInner(0.25);
        //Transition setting 緩衝時間
    ...
    const chart_margin = {top:80,right:40,buttom:40,left:250};
    ...
    let xScale_v1 = d3.scaleLinear().domain(xExtent).range([0,chart_width]);
    //v1: min -> max
    //range : 實際要放東西的地方
    //domain : 資料
...
function chooseData(metric, moviesClean){
    const thisData = moviesClean.sort((a,b)=>b[metric]-a[metric]).filter((d,i)=>i<15); //大到小
    return thisData;
}
